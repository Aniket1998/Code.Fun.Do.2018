...
def get_input_files(corpus_raw):
    book_filenames = sorted(glob.glob("/home/devansh/Downloads/dataset/sentences/*.txt"))
...
def get_pos_files(corpus_raw):
    book_filenames = sorted(glob.glob("/home/devansh/pos_data/*.txt"))
    for book_filename in book_filenames:
        print("Reading '{0}'...".format(book_filename))
        with open(book_filename, "r") as book_file:
            corpus_raw += book_file.read()
...
def get_summary_files(corpus_raw):
    book_filenames = sorted(glob.glob("/home/devansh/Downloads/dataset/catchphrases/catch2/*.txt"))
    for book_filename in book_filenames:
        print("Reading '{0}'...".format(book_filename))
        with open(book_filename, "r") as book_file:
            corpus_raw += book_file.read()
...
# Sentence extraction
print("file read start")
pos_data = get_pos_files(corpus_pos)
print("file read stop")

cleaned_pos_data = sentence_to_wordlist(pos_data)
print cleaned_pos_data[0]
pos_array = {}

for word in cleaned_pos_data:
    pos_array[word[0]] = word[1]

print("createWordList started")
wordList, sentList = createWordList(pos_array, freq, cleaned_tokens)

print("createWordList stoped")

count_model = CountVectorizer(ngram_range = (1, 1))
X = count_model.fit_transform(cleaned_raw_data)

print("np_arrray started")
numpy_array = make_numpy_array(wordList)
print("np_array stoped")

print("output_array_build started")
output_array = output_array_build(wordList)
print("output_array_build stoped")
...
corpus_summary = u""
print("summary read start")
summary_data = get_summary_files(corpus_summary)
print("summary read stop")
...
pickle.dump(clf,open("Bayesian.sav",'w+'))